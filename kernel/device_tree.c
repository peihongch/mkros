...
static int get_prop_of_node(const void *fdt,
	...
	// 解析节点属性的值
	switch (len) {
		case 1: *prop_value = *(uint8_t *)value; break;
		case 2: *prop_value = be16_to_cpu(*(uint16_t *)value); break;
		case 4: *prop_value = be32_to_cpu(*(uint32_t *)value); break;
		default: pr_err("invalid len value of %s: %d", prop, len); break;
	}
...
// 解析 /soc 节点
static int parse_soc_node(const void *fdt, int node) { return 0; }
